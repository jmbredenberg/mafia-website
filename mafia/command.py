...
# symbols which are imported by "from mafia.command import *"
...
from mafia.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("mafia"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="mafia.config")
...
def bootstrap():
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="mafia-site %s" % version)
    ...
    _read_config(args)
    from mafia.model import bootstrap_model
...
def start():
    ...
    from mafia.controllers import Root
